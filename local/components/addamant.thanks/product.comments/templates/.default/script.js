class ProductComment {
  ...
  #selectors = {
    reviewTextarea: document.querySelector(
      `.${this.#blockClasses.reviewTextarea}`
    ),
    reviewTextareaLength: document.querySelector(
      `.${this.#blockClasses.reviewTextareaLength}`
    ),
    btnReviewAddBlock: document.querySelector(
      `.${this.#blockClasses.btnReviewAddBlock}`
    ),
    btnReviewClear: document.querySelector(
      `.${this.#blockClasses.btnReviewClear}`
    ),
  ...
  #addReviewItemsEventHandler() {
    document
      .querySelectorAll(`.${this.#blockClasses.likeBlock}`)
      .forEach((likeBlock) => {
        likeBlock.addEventListener("click", () => {
          const reviewItem = likeBlock.closest(
            `.${this.#blockClasses.reviewItem}`
          );
          ...
          BX.ajax
            ...
            .then((response) => {
              ...
              likeBlock.querySelector(
                `.${this.#blockClasses.likeBlockCount}`
              ).textContent = response.data.COUNT_LIKED;
            });
        });
      });

    document
      .querySelectorAll(`.${this.#blockClasses.btnDeleteReview}`)
      .forEach((btnDeleteReview) => {
        btnDeleteReview.addEventListener("click", () => {
          const reviewItem = btnDeleteReview.closest(
            `.${this.#blockClasses.reviewItem}`
          );
          const commentId = reviewItem.getAttribute("data-comment-id");
          const commentCount = document.querySelectorAll(
