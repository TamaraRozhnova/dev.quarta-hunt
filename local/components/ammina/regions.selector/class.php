...
class AmminaRegionsSelectorComponent extends CBitrixComponent
{
	private $action = '';
	...
	public function onPrepareComponentParams($params)
	{
		...
		if ($params['IP'] === false || strlen($params['IP']) <= 0) {
		...
		if ($params['PREVENT_CITY'] <= 0) {
			if (strlen($_REQUEST['argcity']) > 0) {
				$params['PREVENT_CITY'] = $_REQUEST['argcity'];
			} elseif (strlen($_REQUEST['PREVENT_CITY']) > 0) {
				$params['PREVENT_CITY'] = $_REQUEST['PREVENT_CITY'];
			} else {
				//$params['PREVENT_CITY'] = intval( $app->getContext()->getRequest()->getCookie("ARG_CITY"));
				$params['PREVENT_CITY'] = intval($APPLICATION->get_cookie("ARG_CITY"));
			}
		}
		if ($params['PREVENT_CITY'] <= 0) {
			$params['PREVENT_CITY'] = \Ammina\Regions\BlockTable::getCityIdByIP($params['IP']);
			$params['CONFIRM_REQUEST_SHOW'] = true;
		}
		if ($params['PREVENT_CITY'] <= 0 && $GLOBALS['AMMINA_REGIONS']['SYS_MAIN_CITY_ID'] > 0) {
			$params['PREVENT_CITY'] = $GLOBALS['AMMINA_REGIONS']['SYS_MAIN_CITY_ID'];
			$params['CONFIRM_REQUEST_SHOW'] = true;
		}
		$bNotInDomain = false;
		if ($params['PREVENT_CITY'] > 0) {
			//Проверим - попадает ли выбранный город в обслуживаемые филиалом. если нет - принудительный запрос города
			$iDomain = \Ammina\Regions\DomainTable::doFindDomainByCity($params['PREVENT_CITY']);
			if ($iDomain != $GLOBALS['AMMINA_REGIONS']['SYS_CURRENT_DOMAIN_ID']) {
				$bNotInDomain = true;
				$params['CONFIRM_REQUEST_SHOW'] = true;
			}
		}
		...
		if ($params['PRIORITY_DOMAIN'] === "Y") {
			...
			if ($bNotInDomain) {
				...
				if (intval($APPLICATION->get_cookie("ARG_CITY")) != $params['PREVENT_CITY']) {
		...
		if ($params['ALLOW_REDIRECT'] == "Y" && COption::GetOptionString("ammina.regions", "use_one_domain", "N") != "Y" && $params['PREVENT_CITY'] > 0) {
			$iDomain = \Ammina\Regions\DomainTable::doFindDomainByCity($params['PREVENT_CITY']);
			if ($iDomain != $GLOBALS['AMMINA_REGIONS']['SYS_CURRENT_DOMAIN_ID']) {
				$strLocation = \Ammina\Regions\DomainTable::doGetRedirectLinkByDomainId($iDomain, $params['PREVENT_CITY'], $APPLICATION->GetCurPageParam("", array("")));
...
//myPrint($params);
	...
	protected function prepareAction()
	{
		...
		if (strlen($requestAction) <= 0) {
			$requestAction = $_POST['action'];
		}

		if ($this->isAjaxRequest() && $requestAction === 'getCityForm') {
			$action = 'getCityForm';
		} elseif ($this->isAjaxRequest() && $requestAction === 'setPreventCity') {
	...
	{
		return "/ammina/regions/selector/";
	}
	...
	{
		if ($_REQUEST["PREVENT_CITY"] > 0) {
			$this->setPreventCityCookie($_REQUEST["PREVENT_CITY"]);
		}
	...
	protected function getCityFormAction()
	{
		...
		$this->arResult = array(
			"IS_OK" => true,
			"TITLE" => Loc::getMessage("AMMINA_COMPONENT_REGIONS_FORM_TITLE"),
			"INPUT_PLACEHOLDER" => Loc::getMessage("AMMINA_COMPONENT_REGIONS_INPUT_PLACEHOLDER_TITLE"),
			"ITEMS" => array(),
		);
		/**
		...
		if ($this->arParams['SHOW_CITY_TYPE'] == "D" && !is_array($gps) && strlen($query) <= 0) {
			$this->obtainCityFormDataNoGpsNoQueryDomain();
		} elseif ($this->arParams['SHOW_CITY_TYPE'] == "F" && !is_array($gps) && strlen($query) <= 0) {
			$this->obtainCityFormDataNoGpsNoQuery();
		} elseif (!is_array($gps) && strlen($query) <= 0) {
			$this->obtainCityFormDataNoGpsNoQuery();
		} elseif (!is_array($gps) && strlen($query) > 0) {
	...
	protected function obtainCityFormDataNoGpsNoQueryDomain()
	{
		...
		while ($arDomain = $rDomains->fetch()) {
			$arDomain['NAME'] = \Ammina\Regions\DomainTable::getLangName($arDomain['ID'], LANGUAGE_ID);
		...
		$rCity = \Ammina\Regions\CityTable::getList(array(
			"filter" => $arFilter,
	...
	protected function obtainCityFormDataNoGpsQuery($query)
	{
		...
		$rCity = \Ammina\Regions\CityTable::getList(array(
			...
			"select" => $arSelect,
			"runtime" => array(
				new \Bitrix\Main\Entity\ExpressionField('POSINSTR', 'IF(LOCATE(\'' . mb_strtolower($query) . '\', LOWER(`ammina_regions_city`.`NAME`))>0,LOCATE(\'' . strtolower($query) . '\', LOWER(`ammina_regions_city`.`NAME`)),IF(LOCATE(\'' . strtolower($query) . '\', LOWER(`ammina_regions_city_city_lang`.`NAME`))>0,LOCATE(\'' . strtolower($query) . '\', LOWER(`ammina_regions_city_city_lang`.`NAME`)),1000000000))'),
	...
	protected function obtainCityFormDataGpsNoQuery($fGpsLat, $fGpsLon)
	{
		...
		do {
			...
			$rCity = \Ammina\Regions\CityTable::getList(array(
				...
				"select" => $arSelect,
				"runtime" => array(
					new \Bitrix\Main\Entity\ExpressionField('LENGTH_GPS', 'SQRT(POW(`ammina_regions_city`.`LAT`-' . floatval($fGpsLat) . ',2)+POW(`ammina_regions_city`.`LON`-' . floatval($fGpsLon) . ',2))'),
					new \Bitrix\Main\Entity\ExpressionField('LENGTH_KM_PER_GRAD', 'COS(RADIANS(ABS((`ammina_regions_city`.`LAT`-' . floatval($fGpsLat) . ')/2+' . floatval($fGpsLat) . ')))*40000/360'),
				),
			));
			while ($arCity = $rCity->fetch()) {
				if ($arCity['CITY_ID'] == $this->arParams['PREVENT_CITY']) {
				...
				$arCity['CITY_NAME'] = $arCity['CITY_NAME'] . ", ~" . (round($arCity['LENGTH_GPS'] * $arCity['LENGTH_KM_PER_GRAD'], 1)) . " " . Loc::getMessage("AMMINA_COMPONENT_REGIONS_UNIT_KM");
	...
	protected function obtainCityFormDataGpsQuery($query, $fGpsLat, $fGpsLon)
	{
		...
		do {
			...
			$rCity = \Ammina\Regions\CityTable::getList(array(
				...
				"runtime" => array(
					new \Bitrix\Main\Entity\ExpressionField('LENGTH_GPS', 'SQRT(POW(`ammina_regions_city`.`LAT`-' . floatval($fGpsLat) . ',2)+POW(`ammina_regions_city`.`LON`-' . floatval($fGpsLon) . ',2))'),
					new \Bitrix\Main\Entity\ExpressionField('LENGTH_KM_PER_GRAD', 'COS(RADIANS(ABS((`ammina_regions_city`.`LAT`-' . floatval($fGpsLat) . ')/2+' . floatval($fGpsLat) . ')))*40075.7/360'),
					new \Bitrix\Main\ORM\Fields\ExpressionField('IDD',
						'DISTINCT %s', array("ID")
					)
				),
	...
	{
		$test1 = mb_strtolower($strName);
		$test2 = mb_strtolower($strQuery);
		$iPos = mb_strpos($test1, $test2);
		if ($iPos !== false) {
			$s1 = mb_substr($strName, 0, $iPos);
			$s2 = mb_substr($strName, $iPos, strlen($strQuery));
			$s3 = mb_substr($strName, $iPos + strlen($strQuery));
	...
	{
		$arCity['CITY_NAME'] = CAmminaRegions::getFirstNotEmpty(CityLangTable::getLangNames($arCity['CITY_ID']));
		$arCity['REGION_NAME'] = CAmminaRegions::getFirstNotEmpty(RegionLangTable::getLangNames($arCity['REGION_ID']));
		$arCity['COUNTRY_NAME'] = CAmminaRegions::getFirstNotEmpty(CountryLangTable::getLangNames($arCity['COUNTRY_ID']));
		$arNames = array(
			$arCity['COUNTRY_NAME'],
			$arCity['REGION_NAME'],
			$arCity['CITY_NAME'],
		...
		if (strlen($arCity['CITY_NAME']) > 0 && count($arNames) == 2) {
	...
	{
		global $APPLICATION;
		$APPLICATION->set_cookie('ARG_CITY', $cityId, 3600 * 24 * 365 + time());
		/*
		$app = \Bitrix\Main\Application::getInstance();
	...
	protected function processResultData()
	{
		...
		if ($this->arParams['PREVENT_CITY'] > 0) {
			$this->arResult['CITY_INFO'] = \Ammina\Regions\BlockTable::getCityFullInfoByID($this->arParams['PREVENT_CITY']);
			$arNames = array(
			...
			if (strlen($this->arResult['CITY_INFO']['CITY']['NAME']) > 0 && count($arNames) == 2) {
				$arNames = array_values($arNames);
				unset($arNames[1]);
			} else {
				unset($arNames[2]);
			...
			if (strpos($this->arResult['CITY_NAME_NO_SUBREGION'], '(') !== false) {
				$this->arResult['CITY_NAME_NO_SUBREGION'] = trim(mb_substr($this->arResult['CITY_NAME_NO_SUBREGION'], 0, mb_strpos($this->arResult['CITY_NAME_NO_SUBREGION'], '(') - 1));
			}
		}
	}
