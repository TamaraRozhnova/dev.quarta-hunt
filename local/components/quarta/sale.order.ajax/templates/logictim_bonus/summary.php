					...
					foreach ($arResult["GRID"]["HEADERS"] as $id => $arColumn):
						...
						?>
							<td class="price right">
                                ...
                                <? if($arResult["ARR_BONUS"]["ITEMS"][$arItem["PRODUCT_ID"]]["ADD_BONUS_UNIT"] > 0):?>
                                <div class="add_bonus">
                                	+ <?=$arResult["ARR_BONUS"]["ITEMS"][$arItem["PRODUCT_ID"]]["ADD_BONUS_UNIT"];?> <?=COption::GetOptionString("logictim.balls", "TEXT_BONUS_FOR_ITEM", '')?>
                                </div>
                                <? endif;?>
		...
		<div class="bx_ordercart_order_pay_right">
			<table class="bx_ordercart_order_sum">
				<tbody>
					...
					?>
                    	<tr>
							<td class="custom_t1 fwb" colspan="<?=$colspan?>" class="itog"><?=COption::GetOptionString("logictim.balls", "TEXT_BONUS_PAY", 'pay from bonus:')?></td>
							<td class="custom_t2 fwb" class="price"><?=$arResult["PAY_BONUS_FORMATED"]?></td>
						</tr>
					<?	endif;
					...
					if (strlen($arResult["PAYED_FROM_ACCOUNT_FORMATED"]) > 0)
					{
						?>
						<tr>
							<td class="custom_t1" colspan="<?=$colspan?>" class="itog"><?=GetMessage("SOA_TEMPL_SUM_PAYED")?> n aioo?aiiaai n?aoa</td>
					...
					?>
                     <? if($arResult["ADD_BONUS"] > 0) {
					?>
                    <tr>
						<td class="custom_t1 fwb" colspan="<?=$colspan?>" class="bonuses"><?=COption::GetOptionString("logictim.balls", "TEXT_BONUS_BALLS", 'bonus:')?></td>
						<td class="custom_t2 fwb" class="price"><?=$arResult["ADD_BONUS"]?></td>
					</tr>
                    <? } ?>
				</tbody>
			</table>
