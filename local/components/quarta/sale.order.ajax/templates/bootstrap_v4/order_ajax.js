...
(function () {
  ...
  BX.Sale.OrderAjaxComponent = {
    ...
    init: function (parameters) {
      ...
      this.initialResult = parameters.result || {};
      ...
      this.restrictBlockNode = BX(parameters.restrictBlockId);
    ...
    initPhoneMask: function () {
      ...
      if (phonesOnPage.length > 0) {
        phonesOnPage.forEach((phone, index) => {
          ...
          this.phonesInput[phone.dataset.code] = $(phoneInput).inputmask({
    ...
    refreshOrder: function (result) {
      ...
      if (this.licenseNeed == "Y" && this.paysSystem && this.deliveryIds) {
        if (resultOrder?.DELIVERY) {
          for (let key of Object.keys(resultOrder.DELIVERY)) {
            if (key != this.deliveryIds.DELIVERY_PICKUP_ID) {
              delete result.order.DELIVERY[key];
            }
          }
        }

        if (resultOrder?.PAY_SYSTEM) {
          resultOrder.PAY_SYSTEM.forEach((element, index) => {
            if (
              element?.PAY_SYSTEM_ID == this.paysSystem.CASH_PERSONALLY_ID ||
              element?.PAY_SYSTEM_ID == this.paysSystem.DEBET_CARD_PERSONALLY_ID
            ) {
              tmpResultPays[index] = result.order.PAY_SYSTEM[index];
            }
          });

          tmpResultPays = tmpResultPays.filter(
            (value) => JSON.stringify(value) !== "{}"
          );

          delete result.order.PAY_SYSTEM;

          result.order.PAY_SYSTEM = tmpResultPays;
        }
      }
    ...
    getNewContainer: function (notFluid) {
      return BX.create("DIV", {
        props: { className: "bx-soa-section-content" + (!!notFluid ? "" : "") },
      });
    },
    ...
    editOrder: function () {
      ...
      if (this.licenseNeed == "Y") {
        let selectedDelivery = this.getSelectedDelivery();

        clearTimeout(timer);

        if (selectedDelivery.ID !== "3") {
          timer = setTimeout(() => {
            document.querySelector("#ID_DELIVERY_ID_3").click();
          }, 350);
        } else if (this.result.DELIVERY.length > 1) {
          timer = setTimeout(() => {
            this.sendRequest("refreshOrderAjax");
          }, 350);
        }
      }
    ...
    checkProductsSections: function (result) {
      let restrictedBlock = this.restrictBlockNode;

      if (!result.DELIVERY.length) {
        this.paySystemBlockNode.style.display = "none";
        this.propsBlockNode.style.display = "none";
        this.totalBlockNode.style.display = "none";
        this.orderSaveBlockNode.classList.add("hidden");

        restrictedBlock.style.display = "block";

        Object.values(this.initialResult?.PRODUCTS_RESTRICT_INFO).forEach(
          function (element) {
            if (element.ALLOWED == "N") {
              let listElement = document.createElement("li");
              listElement.innerText = element.NAME;
              restrictedBlock
                .querySelector(".restricted__list")
                .append(listElement);
            }
          }
        );
        ...
        restrictedBlock.querySelector(".restricted__list").innerHTML = "";
        this.orderSaveBlockNode.classList.remove("hidden");
        this.totalBlockNode.style.display = "block";
      }
    },
    ...
    editActiveBasketBlock: function (activeNodeMode) {
      ...
      if (!this.isUserOpt) {
        ...
        setTimeout(() => {
          $("#range")
            .val(this.result.LOGICTIM_BONUS?.PAY_BONUS)
            .css(
              "background-size",
              (this.result.LOGICTIM_BONUS?.PAY_BONUS /
                this.result.LOGICTIM_BONUS?.MAX_BONUS) *
                100 +
                "%"
            );
        }, 100);
      ...
      document.addEventListener("click", (event) => {
        if (
          event.target.classList.contains(
            "bx-soa-coupon-bonus-block-choice-item"
          )
        ) {
          this.unsetActiveBonusBlockChoice();

          event.target.classList.add("active");

          switch (event.target.getAttribute("id")) {
            case "BONUS":
              this.hideBlockChoiceBonusesCoupons("bx-soa-coupon_id");
              this.showBlockChoiceBonusesCoupons(
                "bx-soa-bonus-block-wrapper_id"
              );
              break;
            case "COUPONS":
              this.hideBlockChoiceBonusesCoupons(
                "bx-soa-bonus-block-wrapper_id"
              );
              this.showBlockChoiceBonusesCoupons("bx-soa-coupon_id");
              break;
    ...
    showBlockChoiceBonusesCoupons: function (id) {
      BX.show(BX(id));
    },

    hideBlockChoiceBonusesCoupons: function (id) {
      BX.hide(BX(id));
    },
    ...
    setBlockBonusesCoupons: function () {
      ...
      const blockChoiceAction = BX.create("DIV", {
        ...
        children: [
          BX.create("SPAN", {
            props: {
              className: "bx-soa-coupon-bonus-block-choice-item active",
              id: "BONUS",
            },
            text: "Бонусные баллы",
          }),
          BX.create("SPAN", {
            props: {
              className: "bx-soa-coupon-bonus-block-choice-item",
              id: "COUPONS",
            },
            text: "Промокод",
          }),
      ...
      const blockBonusesCoupons = BX.create("DIV", {
        ...
        children: [blockChoiceAction, bonusBlock],
      });

      BX.append(blockBonusesCoupons, BX("bx-soa-paysystem"));
    ...
    unsetActiveBonusBlockChoice: function () {
      document
        .querySelectorAll(".bx-soa-coupon-bonus-block-choice-item")
        .forEach((item) => {
          item.classList.remove("active");
        });
    },

    unsetActiveBonusBlock: function () {
      document
        .querySelectorAll("#bx-soa-coupon_id,#bx-soa-bonus-block-wrapper_id")
        .forEach((item) => {
          item.classList.remove("active");
        });
    },
    ...
    initSliderBonusTrack: function () {
      const rangeInputs = document.querySelectorAll(
        '.bx-bonus-slider-track input[type="range"]'
      );
      const numberInput = document.querySelector(
        '.bx-bonus-slider-track input[type="text"]'
      );
      ...
      function handleInputChange(e) {
        ...
        if (Number(val) > Number(resultTotalPrice)) {
          target.value = resultTotalPrice;
          val = resultTotalPrice;
        }
    ...
    getSliderBonusTrack: function () {
      ...
      const sliderTrack = BX.create("DIV", {
        ...
        children: [
          BX.create("DIV", {
            props: {
              className: "bx-bonus-slider-track-input-with-btn",
            },
            children: [
              BX.create("INPUT", {
                props: {
                ...
                events: {
                  input: function () {
                    this.applyBonus = document.getElementById("range").value =
                      this.value;
                  },
                },
              }),
            ...
            events: {
              input: function () {
                if (Number(this.value) > Number(resultTotalPrice)) {
                  document.getElementById("paybonus_input").value =
                    resultTotalPrice;
                } else {
    ...
    getBonusesBlock: function () {
      ...
      const bonusBlock = BX.create("DIV", {
        ...
        children: [
          ...
          BX.create("DIV", {
            props: {
              className:
                "bx-soa-bonus-label bx-soa-bonus-user-information-label",
            },
            children: [
              BX.create("LABEL", {
                text: "Сумма",
      ...
      const bonusBlockWithLabel = BX.create("DIV", {
        props: {
          className:
            "bx-block-coupon-bonus-item bx-soa-bonus-block-wrapper active",
          id: "bx-soa-bonus-block-wrapper_id",
        },
        children: [bonusBlock],
      });
    ...
    getBlockCoupons: function () {
      ...
      couponsBlockWithLabel = BX.create("DIV", {
        props: { className: "bx-soa-coupon-wrapper" },
          ...
          if (!BX("bx-soa-coupon_id")) {
            BX.append(
              BX.create("DIV", {
                props: {
                  className:
                    "bx-block-coupon-bonus-item bx-soa-coupon bx-soa-coupon-block-wrapper",
                  id: "bx-soa-coupon_id",
                },
                children: [couponsLabel, couponsBlock],
              }),
    ...
    editPaySystemItems: function (paySystemNode) {
      ...
      var paySystemItemsContainer = BX.create("DIV", {
          props: { className: "bx-soa-pp-item-container pay-items" },
        }),
    ...
    createPaySystemItem: function (item) {
      ...
      itemNode = BX.create("DIV", {
        props: { className: "bx-soa-pp-company col-12 col-sm-6 col-md-4" },
    ...
    editPaySystemInfo: function (paySystemNode) {
      ...
      var paySystemInfoContainer = BX.create("DIV", {
          props: {
            className:
              (this.result.PAY_SYSTEM.length == 0 ? "" : "") +
              "bx-soa-pp-desc-container more-info-items",
          },
        }),
        innerPs,
        extPs,
      ...
      if (errorNode)
        node.appendChild(
          BX.create("div", {
            ...
            children: [errorNode.cloneNode(true)],
          })
        );
    ...
    editDeliveryItems: function (deliveryNode) {
      ...
      var deliveryItemsContainer = BX.create("DIV", {
          props: { className: "bx-soa-pp-item-container delivery-items" },
        }),
    ...
    editDeliveryInfo: function (deliveryNode) {
      ...
      var deliveryInfoContainer = BX.create("DIV", {
          props: { className: "bx-soa-pp-desc-container more-info-items" },
        }),
        currentDelivery,
        logotype,
        name,
          ...
          switch (Number(currentDelivery.ID)) {
            case 3:
              ...
              setTimeout(() => {
                ...
                $(`[name='ORDER_PROP_${property.getSettings().ID}']`).attr(
                  "readonly",
                  "readonly"
                );
            ...
            case 6:
              ...
              setTimeout(() => {
                ...
                if (
                  $(`[for='soa-property-${property.getSettings().ID}']`).find(
    ...
    alterProperty: function (settings, propContainer) {
      ...
      if (divs && divs.length) {
        for (i = 0; i < divs.length; i++) {
          if (BX.hasClass(divs[i], "bx-no-alter-margin")) continue;
    ...
    isValidProperty: function (data, fieldName) {
      ...
      for (i = 0; i < data.inputs.length; i++) {
        ...
        if (
          $(data.inputs[i]).closest(".phone-mask-number-required").length == 1
        ) {
          let currentPhoneDataCode = $(data.inputs[i])
              .closest(".phone-mask-number")
              .attr("data-code"),
            currentPhoneValue =
              this.phonesInput[currentPhoneDataCode][0][
                "inputmask"
              ].unmaskedvalue();
        ...
        if (arProperty.CODE == "ADDRESS") {
          ...
          if (currentDeliveryID.ID == 6 || currentDeliveryID.ID == 13) {
            errors.push(
              field +
                " " +
                BX.message("SOA_REQUIRED") +
                ". " +
                BX.message("SOA_REQUIRED_PLEASE_ADD_POINT")
            );
          } else {
        ...
        this.totalInfoBlockNode.appendChild(
          BX.create("DIV", {
            props: {
              className:
                "bx-soa-cart-total-button-container" +
                (!showOrderButton ? " d-block d-sm-none" : ""),
            },
            children: [
              BX.create("A", {
                props: {
    ...
    editMobileTotalBlock: function () {
      ...
      else BX.addClass(this.mobileTotalBlockNode, "d-none d-sm-none");
    ...
    createTotalUnit: function (name, value, params) {
      ...
      var totalValue,
        className = "bx-soa-cart-total-line " + onlyFree;
