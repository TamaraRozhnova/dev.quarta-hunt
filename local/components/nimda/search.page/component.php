...
if(mb_strlen($arParams["PAGER_TITLE"]) <= 0)
...
if(isset($_REQUEST["tags"]))
	...
	$tags = false;
if(isset($_REQUEST["q"]))
	if(isset($_REQUEST["ajax"]) and $_REQUEST["ajax"]){
//		$q = iconv("utf-8", "windows-1251", trim($_REQUEST["q"]));
		$q = trim($_REQUEST["q"]);
		$arResult["AJAX"] = "Y";
		$arResult["AJAXID"] = $_REQUEST["ajaxid"];
	}else{
		$q = trim($_REQUEST["q"]);
	}
else
...
if($obCache->StartDataCache($arParams["CACHE_TIME"], $this->GetCacheID(), "/".SITE_ID.$this->GetRelativePath()))
{
	...
	foreach($arParams["arrWHERE"] as $code)
	{
		...
		if(mb_strlen($module_id)>0)
		{
			if(mb_strlen($part_id)<=0)
			{
				switch($module_id)
				{
					case "forum":
...
if($this->InitComponentTemplate($templatePage))
{
	...
	if(mb_strlen($folderPath) > 0)
	{
		...
		if(mb_strlen($where)>0)
		{
			...
			if(mb_strlen($part_id)>0) $arFilter["PARAM1"] = $part_id;
		}
		...
		if($obSearch->errorno==0)
		{
			...
			$sArOrder = array();
			$sArFilter = array("ACTIVE" => "Y", "IBLOCK_ID" => CATALOG_IBLOCK_ID);
			$sBIncCnt = false;
			$sSelect = array();
			$sNavStartParams = false;

			$myResult = CIBlockSection::GetList(
				$sArOrder,
				$sArFilter,
				$sBIncCnt,
				$sSelect,
			...
			while ($row = $myResult -> Fetch()) $arResult["SECTIONS"][$row["ID"]] = $row;
...
//				$ar["PREVIEW_PICTURE"] = (0 < $ar["PREVIEW_PICTURE"] ? CFile::GetFileArray($ar["PREVIEW_PICTURE"]) : false);

				$sArOrder = array();
				$sArFilter = array("ID" => $ar["ITEM_ID"], "ACTIVE"=>"Y", "IBLOCK_ID" => CATALOG_IBLOCK_ID);
				$sBIncCnt = false;
				$sSelect = array();
				$sNavStartParams = false;
				...
				$myResult->SetUrlTemplates("/#SECTION_CODE_PATH#/#ELEMENT_CODE#/");

				if($row = $myResult->GetNextElement()) {
					$arItem = $row->GetFields();
					$arItem["PROPERTIES"] = $row->GetProperties();

					$arItem["PREVIEW_PICTURE"] = (0 < $arItem["PREVIEW_PICTURE"] ? CFile::GetFileArray($arItem["PREVIEW_PICTURE"]) : false);
					$arResult["SEARCH_ITEMS"][]=$arItem;
				}
		...
		{
			$sectionPicture = SITE_TEMPLATE_PATH . '/img/no-photo.png';
			if ($row["PICTURE"]) $sectionPicture = CFile::GetPath($row["PICTURE"]);
			$arSectionsPicture[$row["ID"]] = array(
				"SRC" => $sectionPicture
			);
		}
		...
		foreach ($arResult["SEARCH"] as &$arItem)
		{
			...
			{
				if (isset($arSectionsPicture[$arItem["IBLOCK_SECTION_ID"]]))
				{
					$arItem["PREVIEW_PICTURE"] = array("SRC" => $arSectionsPicture[$arItem["IBLOCK_SECTION_ID"]]["SRC"]);
				}
			}
		}

        $arResult["dbg"]["strlen"] = strlen($q);
        $arResult["dbg"]["mb_strlen"] = mb_strlen($q);
