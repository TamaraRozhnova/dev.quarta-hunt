...
class Oneclick extends CBitrixComponent
{
    ...
    protected function API_GetAnonymousUserID($request)
    {
        ...
        if (!$bUserExists) {
            if(strlen(strval(trim($request->get('EMAIL'))))>0){
                ...
                $anonUserID = CSaleUser::DoAutoRegisterUser(
                    ...
                    SITE_ID,
                    $arErrors,
                    array("ACTIVE" => "N", "PHONE_NUMBER" => strval($request->get('PHONE'))
                ));
            }else{
            ...
            if ($anonUserID > 0) {
                ...
                foreach ($arErrors as $arError) {
                    $text_errors.= $arError["TEXT"];
                }
                $this->arResult['validateErrors'][] = ["message" => Loc::getMessage('ERROR_CREATE_ANONYMOUS_USER')." ".$text_errors];
                return 0;
            }
        }
