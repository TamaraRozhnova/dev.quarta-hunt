...
(function() {
	...
	BX.Sale.OrderAjaxComponent.Maps = {
		...
		initializeStoresMaps: function(propsMapData, store_id)
		{
			if (!ymaps)
				return;

			console.log('store_map_'+store_id, {
				center: [propsMapData.lat, propsMapData.lon],
				zoom: propsMapData.zoom
			})
			...
			this.storesMaps[store_id] = new ymaps.Map('store_map_'+store_id, {
				center: [propsMapData.lat, propsMapData.lon],
				zoom: propsMapData.zoom
			});

			this.storesMaps[store_id].behaviors.disable('scrollZoom');

			this.storesMaps[store_id].events.add('click', BX.delegate(function(e){
				var coordinates = e.get('coords'), placeMark;

				if (this.storesMaps[store_id].geoObjects.getLength() === 0)
				{
					placeMark = new ymaps.Placemark([coordinates[0], coordinates[1]], {}, {
						draggable:true,
						preset: 'islands#redDotIcon'
					...
					this.storesMaps[store_id].geoObjects.add(placeMark);
				}
				else
				{
					this.storesMaps[store_id].geoObjects.get(0).geometry.setCoordinates([coordinates[0], coordinates[1]]);
				}
			}, this));
		},
		...
		storesMapsFocusWaiter: function(){},
