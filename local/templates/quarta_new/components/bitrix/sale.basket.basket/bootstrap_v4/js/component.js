;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		bindInitialEvents: function()
		{
			...
			var clearBtn = document.querySelector('.basket__btn-clear');
			BX.bind(clearBtn, 'click', BX.delegate(function() {
				fetch('/ajax/personal/basket.php', {method: 'POST', body: JSON.stringify({action: 'DELETE_ALL'})}).then(function() {
		...
		sendRequest: function(action, data)
		{
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function(result) {
					...
					var prodCount = document.querySelector('.basket__product-count');
					BX.adjust(prodCount, {text: result.PRODUCT_COUNT})
				}, this),
				onfailure: BX.delegate(function() {
					this.actionPool.doProcessing(false);
				}, this)
