window.addEventListener('DOMContentLoaded', () => {
    class Compare {
        ...
        setStyles() {
            this.columnBackdrop.style.width = `${this.containerSize / this.divider}px`;
            this.columnBackdrop.style.transform = `translateX(${this.scrollOffset}px)`;
            const width = (this.containerSize / this.divider) * (this.productsCount + 1);
        ...
        hangDragEvents() {
            this.dragGesture = new DragGesture(this.compareTable, ({ active, movement: [mx], event }) => this.handleDrag(active, mx, event), {
                axis: 'x',
                eventOptions: {
                    capture: true,
                    passive: false
        ...
        hangOnlyDifferentCheckboxEvents() {
            ...
            checkbox.addEventListener('click', (event) => this.handleChangeDifferentCheckbox(event.target.checked));
        }

        handleChangeDifferentCheckbox(newValue) {
            const comparePropsToHide = document.querySelectorAll('.compare__prop[data-different="false"]');
            const value = newValue ?? this.onlyDifferent;
            if (value) {
                comparePropsToHide.forEach(element => element.style.display = 'none');
            } else {
                comparePropsToHide.forEach(element => element.style.display = 'flex');
            }
        ...
        showAddMoreBlock() {
            const compareTableWrapper = document.querySelector('.compare__table-wrapper');
        ...
        deleteElements(productId) {
            const productCardWrapper = document.querySelector(`.compare__item[data-id="${productId}"]`);
            const comparePropColumnElements = document.querySelectorAll(`.compare__prop .compare__col--main[data-id="${productId}"]`);
            const ratingElement = document.querySelector(`.compare__rating[data-id="${productId}"]`);
        ...
        hangProductCardsEvents() {
            ...
            this.productsDataApi.getData(productIds)
                .then(response => {
                    new ProductCards(response, { onDeleteCompare: (productId) => this.handleDeleteCompare(productId) });
                    const ratingElements = document.querySelectorAll('.compare__rating');
                    ratingElements.forEach(element => {
                        new RatingStarsHelper({ productElement: element, ratingsList: response.RATINGS })
                    });
                })
        }
    }
