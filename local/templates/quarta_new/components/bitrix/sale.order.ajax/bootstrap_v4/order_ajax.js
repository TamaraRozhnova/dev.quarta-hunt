...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		refreshOrder: function(result)
		{
			...
			if (
				...
				if (resultOrder?.PAY_SYSTEM) {
					...
					tmpResultPays = tmpResultPays.filter(value => JSON.stringify(value) !== '{}');
		...
		{
			return BX.create('DIV', {props: {className: 'bx-soa-section-content' + (!!notFluid ? '' : '')}});
		},
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			{
				textNode.appendChild(BX.create('span', {props: {className: 'bx-price'}, html: data.PRICE_FORMATED}));
			...
			{
				textNode.appendChild(BX.create('span', {props: {className: 'bx-price all'}, html: data.SUM}));
				if (parseFloat(data.DISCOUNT_PRICE) > 0)
				{
					textNode.appendChild(BX.create('BR'));
			...
			{
				textNode.appendChild(BX.create('span', {props: {className: 'bx-price'}, text: data.DISCOUNT_PRICE_PERCENT_FORMATED}));
			}
			...
			/*bx-text-right */
			return BX.create('DIV', {
				props: {className: 'bx-soa-item-td bx-soa-item-properties ' + (toRight ? '' : '') + column.id},
				children: [
					BX.create('DIV', {
		...
		createBasketItemHiddenColumn: function(column, allData)
		{
			...
			else if (column.id === 'PRICE_FORMATED')
			{
				...
				textNode.appendChild(BX.create('span', {props: {className: 'bx-price all'}, text: data.SUM}));
			else if (column.id === 'DISCOUNT')
				textNode.appendChild(BX.create('span', {props: {className: 'bx-price'}, text: data.DISCOUNT_PRICE_PERCENT_FORMATED}));
				...
				if (event.target.classList.contains('bx-soa-coupon-bonus-block-choice-item')) {
					...
					switch (event.target.getAttribute('id')) {
						case 'BONUS':
							...
							this.showBlockChoiceBonusesCoupons('bx-soa-bonus-block-wrapper_id')
							break;
						case 'COUPONS':
							this.hideBlockChoiceBonusesCoupons('bx-soa-bonus-block-wrapper_id')
		...
		unsetActiveBonusBlockChoice: function() {
			document.querySelectorAll('.bx-soa-coupon-bonus-block-choice-item').forEach( (item) => {
				item.classList.remove('active')
			})
		},

		unsetActiveBonusBlock: function() {
			document.querySelectorAll('#bx-soa-coupon_id,#bx-soa-bonus-block-wrapper_id').forEach( (item) => {
				item.classList.remove('active')
			})
		},
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('INPUT', {
								...
								events: {
								  input: function() {
									this.applyBonus = document.getElementById('range').value = this.value;
								  }
								}
							}),
							BX.create('A', {
								props: {
								  className: 'btn btn-primary selected', id: "btn-choice-apply"
								},
								text: "Применить",	
								events: {
									click: (event) => {
							...
							BX.create('A', {
								props: {
								  className: 'btn btn-primary', id: "btn-choice-reset"
								},
								text: "Сбросить",	
								events: {
									click: (event) => {
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-item-container pay-items'}}),
		...
		editPaySystemInfo: function(paySystemNode)
		{
			...
			var paySystemInfoContainer = BX.create('DIV', {
					props: {
						className: (this.result.PAY_SYSTEM.length == 0 ? '' : '') + 'bx-soa-pp-desc-container more-info-items'
					}
				}),
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-item-container delivery-items'}}),
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryInfoContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-desc-container more-info-items'}}),
