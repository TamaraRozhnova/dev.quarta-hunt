...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			cols = mainColumns.concat(otherColumns);

			checkout__product = BX.create('DIV', {
				props: {className: 'checkout__product'},
				children: cols
			})

			basketItemsNode.appendChild(checkout__product);
		...
		/*
		createBasketItem0: function(basketItemsNode, item, index, active)
		{
			var mainColumns = [],
				otherColumns = [],
				hiddenColumns = [],
		...
		*/
		...
		createBasketItemContent: function(data)
		{
			...
			res  = BX.create('DIV', {props: {className: 'checkout__product-name'}, html: titleHtml});
			return res;
			// return BX.create('DIV', { props: {className: 'checkout__product-name'}, html: titleHtml});
		},
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			var data = allData.columns[column.id] ? allData.columns : allData.data,
				toRight = BX.util.in_array(column.id, ["QUANTITY", "PRICE_FORMATED", "DISCOUNT_PRICE_PERCENT_FORMATED", "SUM"]),
				textNode = BX.create('DIV', {props: {className: 'checkout__product-info'}}), price, discount_price,
				logotype, img;

			if (column.id === 'SUM')
			{

				price = BX.create('STRONG', {props: {className: 'bx-price'}, html: data.SUM});

				if (parseFloat(data.DISCOUNT_PRICE) > 0)
				{
					discount_price = BX.create('SPAN', {props: {className: 'price-old'}, html: data.SUM_BASE_FORMATED});
				}else{
					discount_price = BX.create('SPAN', {props: {className: 'price-old'}, html: ""});
				}
				...
				{
					textNode.appendChild(BX.create('SMALL', {style: 'display:none', text: data.NOTES}));
				}
			}
		...
		*/
		...
		getPersonTypeControl: function(node)
		{
			...
			else if (personTypesCount == 2)
			{
				for (i in this.result.PERSON_TYPE)
				{
					...
					{
						currentType = this.result.PERSON_TYPE[i];
						var inputContainer = BX.create("LABEL", {
							attrs: {className: "form-check checkout__label"},
							children: [
								BX.create('INPUT', {
									attrs: {
										className: "form-check-input",
								...
								BX.create('SPAN', {
									attrs: {
										className: "form-check-label checkout__choose-delivery__text",
		...
		editActiveDeliveryBlock0: function(activeNodeMode)
		{
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			// nimda
			//nimda
			// console.log('pre delivery-self-id')
			//
			// $(BX('bx-soa-order-form')).data('delivery-self-id', this.DEL_SELF_ID);
			// console.log($(BX('bx-soa-order-form')).data('delivery-self-id'), 'delivery-self-id')

			var node = activeNodeMode ? this.deliveryBlockNode : this.deliveryHiddenBlockNode,
				deliveryContent, deliveryNodeRow, deliveryNodeCol, deliveryNodeColEmpty, deliveryNodeRow1, deliveryNode,
				tabs__head, tabs__body,  tabs__content, checkout__tabs, checkout__choose;
			let item;
			let selftab = false;
			let store, checkout__map, checkout__map_left, checkout__map_right;
			let storeMapData;
			...
			else
			{
				...
				console.log('this.result.STORE_LIST', this.result.STORE_LIST)
				...
				for (let k = 0; k < this.deliveryPagination.currentPage.length; k++)
				{
					item = this.deliveryPagination.currentPage[k];

					if(item.ID === this.DEL_SELF_ID && item.CHECKED === 'Y'){
						selftab = true;
					}

					// console.log(this.deliveryPagination.currentPage[k], 'this.deliveryPagination.currentPage[k]', k)

				}
				console.log('selftab', selftab)
				...
				tabs__head = BX.create('DIV', {
					...
					children: [
						BX.create('LABEL', {
							props: {className: 'tabs__item ' + (selftab ? 'is-active' : '')},
							dataset: {tabsItem: "self"},
							children: [
								BX.create('INPUT', {
									props: {type: 'radio', value: 'self', name: "delivery", checked: selftab},
									dataset: {delivery: this.DEL_SELF_ID}
								}),
						...
						BX.create('LABEL', {
							props: {className: 'tabs__item ' + (!selftab ? 'is-active' : '')},
							dataset: {tabsItem: "delivery"},
							children: [
								BX.create('INPUT', {
									props: {type: 'radio', value: 'delivery', name: "delivery", checked: !selftab},
								}),
								BX.create('SPAN', {props: {className: "ui-radio"}}),
				...
				// BX.create('DIV', {props: {className: 'checkout__map-left'}, dataset: {}, children: []}),


				tabs__content = BX.create('DIV', {
					props: {className: 'tabs__content '+(selftab?'is-active':'')},
					dataset: {tabsContent: 'self'}
				});

				if(this.result?.STORE_LIST){
					if(Object.keys(this.result?.STORE_LIST).length > 0){
						for(let i in this.result?.STORE_LIST){
							store = this.result?.STORE_LIST[i];

							checkout__map_left = BX.create('DIV', {props: {className: 'checkout__map-left'}, dataset: {}, children: []});

							if(store?.TITLE){
								checkout__map_left.appendChild(BX.create('DIV', {props: {className: 'checkout__map-title'},
									html: store?.TITLE
								}));
							}

							if(store?.SCHEDULE){
								checkout__map_left.appendChild(BX.create('DIV', {props: {className: 'checkout__map-block'}, children: [
									BX.create('DIV', {props: {className: 'checkout__map-block__name'},
										html: "Время работы магазина:"
									}),
									BX.create('DIV', {props: {className: 'checkout__map-block__desc'},
										html: store?.SCHEDULE
									}),
								]}));
							}
							...
							checkout__map_right = BX.create('DIV', {props: {className: 'checkout__map-right'}, dataset: {}, children: [
									BX.create('DIV', {props: {className: 'map'}, children: [
										BX.create('DIV', {props: {className: 'map-container', id: 'store_map_'+store.ID, style: {width: '100%'}},
												html: ""
										}),
									]}),
								]});
							...
							checkout__map = BX.create('DIV', {
								props: {className: 'checkout__map'},
								dataset: {},
								children: [
									checkout__map_left,
									checkout__map_right,
				...
				tabs__content = BX.create('DIV', {props: {className: 'tabs__content '+(!selftab?'is-active':'')}, dataset: {tabsContent: 'delivery'}});
				...
				this.editDeliveryItems(checkout__choose, 0);
		...
		editDeliveryItems: function(deliveryNode, del_self = 0)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'order-md-1 order-2 col-12 -col-md-7 bx-soa-pp-item-container deliv'}}),
				deliveryItemsContainerRow = BX.create('DIV', {props: {className: 'row1'}}),
				deliveryItemNode, k;
		...
		createDeliveryItem: function(item)
		{
			...
			itemNode = BX.create('LABEL', {
				props: {className: 'bx-soa-pp-company checkout__label ' + (item.ID === this.DEL_SELF_ID?'hidden':'') + (item.CALCULATE_ERRORS || deliveryCached && deliveryCached.CALCULATE_ERRORS ? ' bx-bd-waring' : '')},
		...
		createDeliveryItem0: function(item)
		{
			var checked = item.CHECKED == 'Y',
				deliveryId = parseInt(item.ID),
				labelNodes = [
		...
		selectDeliveryId: function(id)
		{
			console.log($("#ID_DELIVERY_ID_"+id));
			$("#ID_DELIVERY_ID_"+id).parents("label").click();
		...
		initMaps: function()
		{
			...
			if (this.maps)
			{
				...
				if(this.result?.STORE_LIST){
					if(Object.keys(this.result?.STORE_LIST).length > 0){
						for(let i in this.result?.STORE_LIST){
							storeMapData = this.getStoreMapData(this.result?.STORE_LIST[i].ID);
							this.maps.initializeStoresMaps(storeMapData, this.result?.STORE_LIST[i].ID);
						}
					}
				}
			}
		...
		getStoreMapData: function(store_id)
		{
			let currentProperty, locationId, k, long, lat;
			let data = this.options.propertyMap.defaultMapPosition;

			let store, map_id;
			if(this.result?.STORE_LIST){
				if(Object.keys(this.result?.STORE_LIST).length > 0){
					store = this.result?.STORE_LIST[store_id];

					map_id = 'store_map_'+store.ID;

					long = parseFloat(store.GPS_S);
					lat = parseFloat(store.GPS_N);
		...
		editTotalBlock: function()
		{
			...
			var total = this.result.TOTAL,
				...
				showOrderButton = this.params.SHOW_TOTAL_ORDER_BUTTON === 'Y', basket;


			basket = this.totalInfoBlockNode.querySelector('#bx-soa-basket');
			BX.cleanNode(this.totalInfoBlockNode);

			console.log('totalInfoBlockNode', this.totalInfoBlockNode);
			console.log('basket', basket);

			if(basket){
				this.totalInfoBlockNode.appendChild(basket);
			}
		...
		{
			let wrapper = document.querySelector('.bx-soa-cart-total-wrapper')
			...
			scrollTop += $("header.header").height()+parseInt($("header.header").css("top"));

			if (ghostBottom - wrapper.offsetHeight < scrollTop + 20)
				BX.addClass(wrapper, 'bx-soa-cart-total-bottom');
			else
				BX.removeClass(wrapper, 'bx-soa-cart-total-bottom');

			if (scrollTop > ghostTop && !BX.hasClass(wrapper, 'bx-soa-cart-total-fixed'))
			{
				width = wrapper.offsetWidth;
				BX.addClass(wrapper, 'bx-soa-cart-total-fixed');
				this.totalGhostBlockNode.style.paddingTop = wrapper.offsetHeight + 'px';
				wrapper.style.width = width + 'px';
			}
			else if (scrollTop < ghostTop && BX.hasClass(wrapper, 'bx-soa-cart-total-fixed'))
			{
				BX.removeClass(wrapper, 'bx-soa-cart-total-fixed');
				this.totalGhostBlockNode.style.paddingTop = 0;
				wrapper.style.width = '';
			}
		},

		// totalBlockScrollCheck0: function()
		// {
		// 	let wrapper = this.basketBlockNode.querySelector('bx-soa-cart-total')
		// 	if (!this.totalInfoBlockNode || !this.totalGhostBlockNode)
		// 		return;
		//
		// 	var scrollTop = BX.GetWindowScrollPos().scrollTop,
