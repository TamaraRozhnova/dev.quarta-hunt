(function (window) {
  ...
  window.JCCatalogElement.prototype = {
    ...
    bindSizeChangeLoggler: function () {
      let switchers = document.querySelectorAll(".element__sku-switcher");
      if (switchers.length) {
        switchers.forEach((element) => {
          element.addEventListener("click", function (evt) {
            evt.preventDefault();
            let aim = document.querySelectorAll(
              "." + this.getAttribute("aim") + " li"
            );
            ...
            aim.forEach((size) => {
              if (this.classList.contains("rus")) {
                size.querySelector(".product-item-scu-item-text").textContent =
                  size.getAttribute("alternate");
              } else {
                size.querySelector(".product-item-scu-item-text").textContent =
                  size.getAttribute("title");
              }
            });
          });
        });
    ...
    initConfig: function () {
      ...
      this.config.showSlider = false; //this.params.CONFIG.SHOW_SLIDER === "Y";
...
BX.ready(function () {
  if (window.innerWidth > 450 && typeof Fancybox !== "undefined") {
    ...
    let sliders = document.querySelectorAll(".element__galery-item");
    let sliderWrap = document.querySelector(".element__galery-list");
    let sliderClose = document.querySelector(".element__galery-close");
    if (sliders.length) {
      sliders.forEach((element) => {
        element.addEventListener("click", function (evt) {
          evt.preventDefault();
          let video = document.querySelector(
            ".element__galery-item--video video"
          );
          sliderWrap.classList.add("open");
          if (video) {
            video
              .closest(".element__galery-item--video")
