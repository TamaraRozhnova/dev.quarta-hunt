window.addEventListener("DOMContentLoaded", () => {
  class ContactsForm {
    ...
    async handleSubmit(event) {
      ...
      console.log(
        Request.fetchWithFormData(this.ajaxUrl, this.getDataForSubmit())
      );
      ...
      try {
        const response = await Request.fetchWithFormData(
          this.ajaxUrl,
          this.getDataForSubmit()
        );
        if (!response) {
          throw new Error();
        }
    ...
    scrollToSuccessElement() {
      const successBlock = document.querySelector(
        ".contacts-form__success-message"
      );
    ...
    showError(errorText) {
      this.form.insertAdjacentHTML(
        "afterbegin",
        this.createErrorHtml(errorText)
      );
    }
    ...
    createControls() {
      ...
      Object.keys(this.inputs).forEach((key, value) => {
        if (this.inputs[key].required == true) {
          this.requiredInputs.push(this.inputs[key]);
        }
      });
    }
  }
